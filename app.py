...
def build_project_record(answers, scores):
    ...
    return {
        ...
        "date": str(st.session_state.get("eval_date", date.today())),
...
def upsert_project_record(record):
    records = load_saved_projects()
    new_key = project_key(record.get("project_name", ""), record.get("region", ""))

    records = [
        r for r in records
        if project_key(r.get("project_name", ""), r.get("region", "")) != new_key
    ]
...
def render_portfolio_chart():
    ...
    st.markdown(
        '<div class="small-note">X-axis = Project Viability (%), Y-axis = Delphy Chance to Win (%), bubble size = H₂ quantity to store (tonnes).</div>',
        unsafe_allow_html=True
    )
    ...
    chart_df["size_plot"] = chart_df["h2_quantity_tonnes"].fillna(0.0).clip(lower=0.0)

    if chart_df["size_plot"].max() <= 0:
        chart_df["size_plot"] = 1.0
    else:
        min_nonzero = max(chart_df["size_plot"].max() * 0.10, 0.3)
        chart_df["size_plot"] = chart_df["size_plot"].apply(lambda x: x if x > 0 else min_nonzero)
...
with col_form:
    ...
    if h2_src == "Produced on site":
        ...
        if not ele_unknown:
            st.number_input("Electrolyzer size (MW)", min_value=0.0, step=1.0, key="electrolyzer_mw")
        else:
        ...
        st.number_input("Tonnes of H₂ to store", min_value=0.0, step=0.5, key="h2_qty_onsite")
        ...
        if not flow_unknown:
            st.number_input("Flowrate (kg/day)", min_value=0.0, step=100.0, key="flowrate_kgday")
        else:
        ...
        st.markdown(f'**10. Estimate the tons of H₂ likely to be stored?** {dtag()}', unsafe_allow_html=True)
        st.number_input("Tonnes of H₂ to store", min_value=0.0, step=0.5, key="h2_qty_pipeline")
        ...
        st.number_input("Tonnes of H₂ to store", min_value=0.0, step=0.5, key="h2_qty_other")
        render_view_modify("q10", label="View/Modify Score")
...
with col_result:
    ...
    if submitted:
        if project_name:
            ...
            st.success(f"✅ Evaluation saved for **{project_name}**.")
        else:
    ...
    if not saved_df.empty:
        ...
        delete_options = [
            f"{row['project_name']} | {row['region']}"
            for _, row in saved_df[["project_name", "region"]].drop_duplicates().iterrows()
        ]
        selected_delete = st.selectbox(
            "Delete saved project",
            options=["-- Select project --"] + delete_options,
            index=0
        )

        if st.button("🗑️ Delete Selected Project", use_container_width=True):
            if selected_delete != "-- Select project --":
                name_part, region_part = selected_delete.split(" | ", 1)
                delete_saved_project(name_part, region_part)
                st.success(f"Deleted saved project: {name_part}")
                st.rerun()
    else:
